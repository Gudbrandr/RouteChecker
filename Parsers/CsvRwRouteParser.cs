...
namespace OpenBve {
	internal class CsvRwRouteParser {
		...
		// Custom event handler to pass progress to the main form.
...
//				Game.Sections[0].Trains = new TrainManager.Train[] { };
			...
			ParseRouteForData(FileName, IsRW, Encoding, TrainPath, ObjectPath, SoundPath, ref Data, PreviewOnly, DikeReport, PoleReport, RailReport, WallReport);
...
//			ApplyRouteData(FileName, Encoding, ref Data, PreviewOnly);
		...
		//	Expressions for conversion from rw to csv. Only saved for rw files.
		public static Expression[] rwExpressions;
		
		private static void ParseRouteForData(string FileName, bool IsRW, System.Text.Encoding Encoding, string TrainPath, string ObjectPath, string SoundPath, ref RouteData Data, bool PreviewOnly, bool DikeReport, bool PoleReport, bool RailReport, bool WallReport) {
			// parse
			...
			ParseRouteForData(FileName, IsRW, Encoding, Expressions, TrainPath, ObjectPath, SoundPath, UnitOfLength, ref Data, PreviewOnly, DikeReport, PoleReport, RailReport, WallReport);
...
//							RouteChecker.Program.MinimumJumpToPositionValue = p[j].TrackPosition;
		...
		// Rail monitor class.
		private	enum railState{Started, Modified, Ended, Autostarted};
		private class RailMon
		{
			internal int Line;
			internal railState State = railState.Ended;
		}
		private class PoleMon
		{
			internal int Line;
			internal railState State = railState.Ended;
		}
		private class WallMon
		{
			internal int Line;
			internal railState State = railState.Ended;
		}
		private class DikeMon
		{
			internal int Line;
			internal railState State = railState.Ended;
		}
		...
		// parse route for data
		private static void ParseRouteForData(string FileName, bool IsRW, System.Text.Encoding Encoding, Expression[] Expressions, string TrainPath, string ObjectPath, string SoundPath, double[] UnitOfLength, ref RouteData Data, bool PreviewOnly, bool DikeReport, bool PoleReport, bool RailReport, bool WallReport) {
			System.Globalization.CultureInfo Culture = System.Globalization.CultureInfo.InvariantCulture;
			...
			double progressFactor = Expressions.Length == 0 ? 0.5 : 0.5 / (double)Expressions.Length;
			
			DikeMon[] DikeMons = new DikeMon[256];
			PoleMon[] PoleMons = new PoleMon[256];
			RailMon[] RailMons = new RailMon[256];
			WallMon[] WallMons = new WallMon[256];
			if(DikeReport){
				for(int ii = 0;ii < DikeMons.Length;ii++ ){
					DikeMons[ii] = new DikeMon();
				}
			}
			if(PoleReport){
				for(int ii = 0;ii < PoleMons.Length;ii++ ){
					PoleMons[ii] = new PoleMon();
				}
			}
			if(RailReport){
				for(int ii = 0;ii < RailMons.Length;ii++ ){
					RailMons[ii] = new RailMon();
				}
				// Track 0 is considered to exist if no explicit start command appears for it.
				RailMons[0].State = railState.Autostarted;
			}
			if(WallReport){
				for(int ii = 0;ii < WallMons.Length;ii++ ){
					WallMons[ii] = new WallMon();
				}
			}
...
//				Loading.RouteProgress = (double)j * progressFactor;
				// Raise the custom progress event.
				if(onProgress != null){
					onProgress(null, new ProgressEventArgs((double)j * progressFactor));
				}
								...
								case "train.timetable.day":
									{
										if (!PreviewOnly) {
											...
											// Code was always unreachable because this is routechecker //
...
//				Loading.RouteProgress = 0.3333 + (double)j * progressFactor;
				// Raise the custom progress event.
				if(onProgress != null){
					onProgress(null, new ProgressEventArgs(0.5 + (double)j * progressFactor));
				}
								...
								case "track.rail":
									{
										if (!PreviewOnly) {
											...
											if (idx < 1) {
												...
												// Rail monitor.
												if(RailReport){
													// Rail 0 can't be started manually. Index must be > 0 or is caught above.
													if(RailMons[idx].State == railState.Started && Command.ToLowerInvariant() == "track.railstart"){
														string strMsg = ". Already started at line " + RailMons[idx].Line + ".";
														Interface.AddMessage(Interface.MessageType.Error, false, "Rail " + idx + " started at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
													}else if(RailMons[idx].State == railState.Ended){
														Interface.AddMessage(Interface.MessageType.Information, false, "Rail " + idx + " started at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														RailMons[idx].State = railState.Started;
														RailMons[idx].Line = Expressions[j].Line;
													}else if(RailMons[idx].State == railState.Modified && Command.ToLowerInvariant() == "track.railstart"){
														string strMsg = ". Last modified at line " + RailMons[idx].Line + ".";
														Interface.AddMessage(Interface.MessageType.Error, false, "Rail " + idx + " started at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
													}else if(RailMons[idx].State == railState.Modified && Command.ToLowerInvariant() == "track.rail"){
														Interface.AddMessage(Interface.MessageType.Information, false, "Rail " + idx + " modified at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														RailMons[idx].State = railState.Modified;
														RailMons[idx].Line = Expressions[j].Line;
													}else if((RailMons[idx].State == railState.Started || RailMons[idx].State == railState.Autostarted) && Command.ToLowerInvariant() == "track.rail"){
														Interface.AddMessage(Interface.MessageType.Information, false, "Rail " + idx + " modified at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														RailMons[idx].State = railState.Modified;
														RailMons[idx].Line = Expressions[j].Line;
                                                    }
								...
								case "track.railend":
									{
										if (!PreviewOnly) {
											...
											// Rail monitor.
											if(RailReport){
												if(RailMons[idx].State == railState.Started || RailMons[idx].State == railState.Autostarted || RailMons[idx].State == railState.Modified){
													Interface.AddMessage(Interface.MessageType.Information, false, "Rail " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
													RailMons[idx].State = railState.Ended;
													RailMons[idx].Line = Expressions[j].Line;
												}else if(RailMons[idx].State == railState.Ended){
													string strMsg = ". Already ended at line " + RailMons[idx].Line + ".";
													Interface.AddMessage(Interface.MessageType.Error, false, "Rail " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
												}
											}
										}
									} break;
								case "track.railtype":
									{
										if (!PreviewOnly) {
											...
											if (idx < 0) {
												...
												if(RailReport){
													if(RailMons[idx].State == railState.Started || RailMons[idx].State == railState.Autostarted || RailMons[idx].State == railState.Modified){
														Interface.AddMessage(Interface.MessageType.Information, false, "Rail " + idx + " type changed at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														RailMons[idx].State = railState.Modified;
														RailMons[idx].Line = Expressions[j].Line;
													}else if(RailMons[idx].State == railState.Ended){
														string strMsg = ". Already ended at line " + RailMons[idx].Line + ".";
														Interface.AddMessage(Interface.MessageType.Error, false, "Rail " + idx + " type changed at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
													}
												}
											}
										}
										...
										if (!PreviewOnly) {
											...
											if (idx < 0) {
												...
												if (typ < 0 || typ >= Data.Structure.Poles.Length || Data.Structure.Poles[typ] == null) {
													...
													// Pole monitor.
													if(PoleReport){
														if(PoleMons[idx].State == railState.Ended){
															Interface.AddMessage(Interface.MessageType.Information, false, "Pole " + idx + " started at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
															PoleMons[idx].State = railState.Started;
															PoleMons[idx].Line = Expressions[j].Line;
														}else if(PoleMons[idx].State == railState.Started || PoleMons[idx].State == railState.Modified){
															Interface.AddMessage(Interface.MessageType.Information, false, "Pole " + idx + " modified at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
															PoleMons[idx].State = railState.Modified;
															PoleMons[idx].Line = Expressions[j].Line;
														}
													}
												}
								...
								case "track.poleend":
									{
										if (!PreviewOnly) {
											...
											if (idx < 0 | idx >= Data.Blocks[BlockIndex].RailPole.Length) {
												...
												// Pole monitor.
												if(PoleReport){
													if(PoleMons[idx].State == railState.Started || PoleMons[idx].State == railState.Modified){
														Interface.AddMessage(Interface.MessageType.Information, false, "Pole " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														PoleMons[idx].State = railState.Ended;
														PoleMons[idx].Line = Expressions[j].Line;
													}else if(PoleMons[idx].State == railState.Ended){
														string strMsg = ". Already ended at line " + PoleMons[idx].Line + ".";
														Interface.AddMessage(Interface.MessageType.Error, false, "Pole " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
													}
												}
											}
										}
										...
										if (!PreviewOnly) {
											...
											if (dir <= 0 && (sttype >= Data.Structure.WallL.Length || Data.Structure.WallL[sttype] == null) ||
												...
												if (idx < 0) {
													...
													// Wall monitor.
													if(WallReport){
														if(WallMons[idx].State == railState.Ended){
															Interface.AddMessage(Interface.MessageType.Information, false, "Wall " + idx + " started at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
															WallMons[idx].State = railState.Started;
															WallMons[idx].Line = Expressions[j].Line;
														}else if(WallMons[idx].State == railState.Started || WallMons[idx].State == railState.Modified){
															Interface.AddMessage(Interface.MessageType.Information, false, "Wall " + idx + " modified at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
															WallMons[idx].State = railState.Modified;
															WallMons[idx].Line = Expressions[j].Line;
														}
													}
												}
								...
								case "track.wallend":
									{
										if (!PreviewOnly) {
											...
											if (idx < 0 | idx >= Data.Blocks[BlockIndex].RailWall.Length) {
												...
												// Wall monitor.
												if(WallReport){
													if(WallMons[idx].State == railState.Started || WallMons[idx].State == railState.Modified){
														Interface.AddMessage(Interface.MessageType.Information, false, "Wall " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														WallMons[idx].State = railState.Ended;
														WallMons[idx].Line = Expressions[j].Line;
													}else if(WallMons[idx].State == railState.Ended){
														string strMsg = ". Already ended at line " + WallMons[idx].Line + ".";
														Interface.AddMessage(Interface.MessageType.Error, false, "Wall " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
													}
												}
											}
										}
										...
										if (!PreviewOnly) {
											...
											if (dir <= 0 && (sttype >= Data.Structure.DikeL.Length || Data.Structure.DikeL[sttype] == null) ||
												...
												if (idx < 0) {
													...
													// Dike monitor.
													if(DikeReport){
														if(DikeMons[idx].State == railState.Ended){
															Interface.AddMessage(Interface.MessageType.Information, false, "Dike " + idx + " started at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
															DikeMons[idx].State = railState.Started;
															DikeMons[idx].Line = Expressions[j].Line;
														}else if(DikeMons[idx].State == railState.Started || DikeMons[idx].State == railState.Modified){
															Interface.AddMessage(Interface.MessageType.Information, false, "Dike " + idx + " modified at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
															DikeMons[idx].State = railState.Modified;
															DikeMons[idx].Line = Expressions[j].Line;
														}
													}
												}
								...
								case "track.dikeend":
									{
										if (!PreviewOnly) {
											...
											if (idx < 0 | idx >= Data.Blocks[BlockIndex].RailDike.Length) {
												...
												// Dike monitor.
												if(DikeReport){
													if(DikeMons[idx].State == railState.Started || DikeMons[idx].State == railState.Modified){
														Interface.AddMessage(Interface.MessageType.Information, false, "Dike " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + Expressions[j].File);
														DikeMons[idx].State = railState.Ended;
														DikeMons[idx].Line = Expressions[j].Line;
													}else if(DikeMons[idx].State == railState.Ended){
														string strMsg = ". Already ended at line " + DikeMons[idx].Line + ".";
														Interface.AddMessage(Interface.MessageType.Error, false, "Dike " + idx + " ended at line " + Expressions[j].Line.ToString(Culture) + ", column " + Expressions[j].Column.ToString(Culture) + " in file " + FileName + strMsg);
													}
												}
											}
										}
...
//														Data.Blocks[BlockIndex].Sound[n].Radius = radius;
			...
			// If this is an RW, save the expressions for conversion.
			if(IsRW){
				rwExpressions = Expressions;	
			}
		}
